                ...
                for p in text.findChildren():
                    if has_class(p, "tweet-text"):
                        if hasattr(p, "contents") and not isinstance(p.contents[0], type(p)):
                            #print(p.contents)
                            last_i = 0
                            for i in reversed(range(len(p.contents))):
                                if "<strong>" in str(p.contents[i]) and "</strong>" in str(p.contents[i]):
                                    last_i = i
                                    #print("!!!!!!!!!!!!!" + str(i))
                                    break

                            text_tmp = ""
                            p.contents = p.contents[:last_i+2]
                            #print(p.contents)
                            for item in p.contents:
                                if "<strong>" in str(item) and "</strong>" in str(item):
                                    text_tmp += str(item)[8:-9]
                                else:
                                    text_tmp += str(item)

                            tweet.text = clean_text(text_tmp)
                            #print(tweet.text)
                            #tweet.text = clean_text(p.contents[0]) #p.contents[0]
                            #tweet.text = tweet.text.append(clean_text(p.contents[1]))
